...
description: "55 Tech Scraping API — 通过 80 多个地理分布式代理节点路由 HTTP、浏览器渲染、WebSocket 和 AMQP 请求。"
---
...
- **浏览器渲染** — 使用真实 Chromium 浏览器渲染页面，支持 JS 执行、Cookie 捕获（包括 httpOnly）、截图
...
- **代理链接** — 通过住宅代理路由浏览器请求，兼具数据中心速度和住宅 IP 指纹
- **响应验证** — 验证浏览器渲染页面中是否存在预期内容，缺失时自动在不同节点重试
...
| `/browser` | POST | In body | 使用真实浏览器渲染页面（JS、Cookie、截图、代理、验证） |
