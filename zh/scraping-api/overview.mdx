...
**Scraping API** 由 [55 Tech](https://55-tech.com/) 提供，可将您的 HTTP、WebSocket 和 AMQP 请求通过由 60 多个地理分布式代理节点组成的网络进行路由。请求会自动路由到健康的节点，无需您自行管理代理即可获得可靠的响应。
...
通过 `X-API-Key` 请求头、`apiKey` 查询参数或（对于 WebSocket/AMQP）`apiKey` 请求体字段传递您的 API 密钥。详见 [Authentication](/zh/scraping-api/authentication)。
...
| `X-Agent` | `?agent=` query param | 通过标识路由到特定代理节点，或逗号分隔列表 | `de1`, `de1,at5,us3` |
