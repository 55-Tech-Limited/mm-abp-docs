...
| `X-Block-Resources` | 否 | —（`/browser`），`image,font,stylesheet,media`（`/browser/stream`） | 阻止的资源类型。流模式默认阻止以节省带宽。设置为 `none` 可禁用 |
...
仅捕获数据请求（XHR/Fetch API 调用）— 完整 HTML 页面和 JavaScript 包会被自动过滤。默认阻止图片、字体、样式表和媒体以节省带宽。所有数据通过 gzip 压缩传输。
