...
| `subprotocols` | array | No | `[]` | WS 子协议协商。作为 `Sec-WebSocket-Protocol` 请求头发送。例如 `["centrifuge-protobuf"]` |
| `proxy` | string | No | `""` | WebSocket 连接的代理 URL（`http://` 或 `socks5://`）。适用于通过住宅或 ISP 代理进行路由。 |
