...
Scraping API 可以通过代理网络中继双向 WebSocket 连接。代理节点使用 `curl_cffi AsyncWebSocket` 配合 TLS 指纹模拟连接到目标，因此目标看到的是真实的浏览器握手。所有 WS 子协议均透明中继 — Socket.IO、SignalR、Centrifugo、GraphQL-WS、原生 WebSocket 等。
...
| `idle_timeout` | float | No | `0` | 等待目标消息的超时时间（秒）。`0` = 不超时 |
  ...
  <Step title="接收连接确认">
    网关通过调度器选择代理节点，代理节点使用 TLS 指纹模拟连接到目标 WebSocket，然后网关响应：
...
通过代理网络从 RabbitMQ 代理流式传输消息，以 Server-Sent Events 形式交付。代理节点使用 `aio-pika` 连接到消息代理，创建临时自动删除队列，将其绑定到指定的交换机，并将消息流式传回。
