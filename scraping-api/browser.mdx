...
| `X-Block-Resources` | No | — (`/browser`), `image,font,stylesheet,media` (`/browser/stream`) | Resource types to block. Stream blocks by default for bandwidth. Set to `none` to disable |
...
Only data-carrying requests (XHR/Fetch API calls) are captured — full HTML pages and JavaScript bundles are filtered out. Images, fonts, stylesheets, and media are blocked by default to minimize bandwidth. All data is gzip-compressed over the wire.
...
Same headers as `/browser` (except `X-Timeout` defaults to `0` = runs until you disconnect, 24h safety cap), plus:
