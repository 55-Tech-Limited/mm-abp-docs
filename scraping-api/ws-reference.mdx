...
The WebSocket relay at `wss://scraping-api.55-tech.com/ws` provides a bidirectional proxy. You connect to the gateway, send a JSON connect message, and the gateway relays all frames between you and the target through a TLS-fingerprinted agent.

```
Client ←→ Gateway (wss://.../ws) ←→ Agent (gRPC WsRelay) ←→ Target WebSocket
```

## Interactive playground

The API includes a built-in Swagger UI with WebSocket documentation:

```
https://scraping-api.55-tech.com/docs
```
