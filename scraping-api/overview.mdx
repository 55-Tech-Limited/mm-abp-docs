...
The **Scraping API** by [55 Tech](https://55-tech.com/) lets you route HTTP, WebSocket, and AMQP requests through a network of 60+ geo-distributed agents. Requests are automatically routed to healthy nodes, so you get reliable responses without managing proxies yourself.
...
Pass your API key via the `X-API-Key` header, the `apiKey` query parameter, or (for WebSocket/AMQP) the `apiKey` body field. See [Authentication](/scraping-api/authentication) for details.
...
| `X-Agent` | `?agent=` query param | Route through specific agent(s) by slug or comma-separated list | `de1`, `de1,at5,us3` |
