...
The Scraping API can relay bidirectional WebSocket connections through the agent network. The agent connects to the target using `curl_cffi AsyncWebSocket` with TLS fingerprinting on the upgrade request, so the target sees a real browser handshake. All WS sub-protocols are relayed transparently — Socket.IO, SignalR, Centrifugo, GraphQL-WS, raw WebSocket, etc.
...
| `idle_timeout` | float | No | `0` | Seconds to wait for a target message before disconnect. `0` = no timeout |
  ...
  <Step title="Receive connected confirmation">
    The gateway picks an agent via the scheduler, the agent connects to the target WebSocket with TLS fingerprinting, and the gateway responds with:
...
Stream messages from a RabbitMQ broker through the agent network, delivered as Server-Sent Events. The agent connects to the broker using `aio-pika`, creates a temporary auto-delete queue, binds it to the specified exchange, and streams messages back.
...
```
event: connected
data: {"agent_id": "scraping-de1", "queue_name": "scrape_de1_4821"}
```
