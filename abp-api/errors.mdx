...
# Include the header
curl -H "X-ABP-API-KEY: your-key" https://v2.55-tech.com/accounts
# Returns: 200
```
...
In rare cases, the system may enter emergency mode (triggered by data staleness or upstream failures). During emergency mode:

- New orders are rejected
- Existing pending orders may be expired
- The `emergency` WebSocket channel broadcasts status changes
- Recovery is automatic (soft pause) or manual (full emergency)
